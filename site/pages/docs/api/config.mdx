...
- **Type:** `boolean | { models?: ('chatgpt' | 'claude' | 'gemini')[], query?: ((p: { location: string }) => string) }`
- **Default:** `true`

Whether or not to show the AI call-to-action dropdown (ie. "Ask in ChatGPT").
...
You can choose which AI providers appear in the dropdown via `models`. The first entry becomes the primary button and the rest appear in the dropdown. Omit to show all providers with ChatGPT as primary.
...
export default defineConfig({
  aiCta: { // [!code focus]
    models: ['claude', 'gemini'], // [!code focus]
  }, // [!code focus]
  title: 'Viem'
})
```
