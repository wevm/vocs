...
Use `x-displayName` when a tag needs an internal identity that differs from its sidebar label. Set `x-pagePath` to customize its route below the OpenAPI mount, including nested paths.

```yaml [openapi.yaml]
tags:
  - name: Funding Transfers
    x-displayName: Transfers # [!code focus]
    x-pagePath: funding/transfers # [!code focus]
```

With an OpenAPI mount at `/api`, this tag renders as **Transfers** at `/api/funding/transfers`. Operations continue to reference the tag's `name`, so another tag can use the same display name without combining their operations.
