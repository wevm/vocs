...
At build time, Vocs walks `src/pages/`, splits each MDX page into title/subtitle/text/category fields, and builds a frozen full-text index ([FrozenMiniSearch](https://github.com/yoch/frozenminisearch)). The index is written to `assets/search-index-{hash}.json` for production and served as a virtual module in dev (rebuilt on MDX changes).
