...
- **`Retriever.local`**: Vocs owns the whole pipeline. It chunks and embeds your pages at build time into a built-in static vector store, then searches it at runtime. The open-source, self-owned option.
- **`Retriever.cloudflare`** (or any `Retriever.Adapter`): Vocs delegates retrieval to a managed backend (e.g. Cloudflare AI Search) and fuses the results with keyword search. Use this when you'd rather not own the index.
...
### Set up built-in AI search

Enable the self-owned `Retriever.local` provider with an embedding provider. This example uses OpenAI.

::::steps

#### Add a local retriever

Point `ai.retriever` at `Retriever.local` with an embedding adapter. Vocs reads the API key from the environment.
...
Ship as usual. The server endpoint (`/api/search` by default) embeds each query and searches the packed vector store. If the index is still warming up, the dialog quietly falls back to keyword results.
...
To delegate retrieval to a managed backend instead of owning the index, set `ai.retriever` to a managed adapter like `Retriever.cloudflare`. Vocs queries it at runtime (`/api/search`) and fuses the results with keyword search.

```ts twoslash
// [!code word:cloudflare]
...
This needs no build-time embedding step, since indexing happens on the managed service.
...
<Cards>
  ...
  <Card
    title="ai.retriever"
    description="Full AI search config: local provider, embedding, reranker, sources, hybrid, chunking, cache."
    icon="settings"
    to="/reference/site-config#airetriever"
  />
  <Card
    title="Retriever.cloudflare"
    description="Managed retriever config and the Cloudflare AI Search adapter."
    icon="cloud"
    to="/reference/site-config#retrievercloudflare"
  />
  <Card
