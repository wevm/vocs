...
- The embedding provider's credentials (e.g. `OPENAI_API_KEY`), since each query is embedded on the fly.
...
To delegate retrieval to a managed backend instead of owning the index, set `ai.retriever` to a managed adapter like `Retriever.cloudflare`. Vocs queries it at runtime (`/api/search`) and blends the results with keyword search (appended below by default; enable `hybrid` to fuse them into a single ranking).
