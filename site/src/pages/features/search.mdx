...
There is no build-time embedding step; indexing happens on the hosted service. Keep the credentials available to the server at runtime. Each `/api/search` request runs one AI Search query against your account, so the same rate-limiting note as the local retriever applies.
