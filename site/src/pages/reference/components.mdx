import { Badge, Callout, Card, Cards, FeedbackWidget, HomePage, Link, Prompt, Sandbox, Tab, Tabs } from 'vocs'
...
Use `Prompt` to render copyable instructions for an AI agent from JSX or MDX. Hide the ChatGPT and Claude buttons site-wide with [`askAi.openIn`](/reference/site-config#askai).

:::code-group
<div data-title="Preview">
  <Prompt value={'Read https://vocs.dev/introduction/getting-started and update `vocs.config.ts`.'} />
</div>
```tsx [Code]
...
export function AgentInstructions() {
  return (
    <Prompt
      value={'Read https://vocs.dev/introduction/getting-started and update `vocs.config.ts`.'}
    />
  )
}
```
