...
- **Type:** `(string | { url: string; label?: string; weight?: number })[]`

External source URLs to embed alongside local docs. Each is fetched at build time and auto-expanded:
a `sitemap.xml` → every page it lists, an `llms.txt` → every page it links, anything else → the page
itself. Results link out to their absolute URL. `label` sets the result badge (defaults to the
hostname); `weight` is a score multiplier relative to local docs (which are `1`), defaulting to
`0.9`.
...
Where retrieval runs. `'auto'` resolves to `'server'`.
...
retrieval to the backend (queried at runtime, then fused with keyword search) with no build-time
