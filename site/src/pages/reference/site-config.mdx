...
Adds built-in retrieval-augmented (RAG) AI search on top of keyword search. Vocs embeds your
pages at build time, packs them into a built-in static vector store, and queries them at runtime via
a server endpoint. Embedding/reranker adapters hold secrets and are kept **server-side only** (never
serialized to the browser). For a managed backend instead, use [`search.retriever`](#searchretriever).
...
- **Required** (unless using the `rag: true` shorthand, which defaults to `Embedding.ollama()`)
...
#### `search.rag.retrieval.candidateK`

- **Type:** `number`
- **Default:** `40`

Number of candidates scored before dedupe/rerank.

#### `search.rag.retrieval.hybrid`

- **Type:** `boolean`
- **Default:** `false`

Fuse keyword (MiniSearch) and AI results into a single ranking. When `false`, AI results
are appended to keyword results instead.

#### `search.rag.retrieval.semanticWeight`

- **Type:** `number`
- **Default:** `0.7`

Weight applied to AI results during hybrid fusion.

#### `search.rag.retrieval.keywordWeight`

- **Type:** `number`
- **Default:** `0.3`

Weight applied to keyword results during hybrid fusion.

#### `search.rag.chunking.maxCharacters`
...
#### `search.rag.chunking.overlapCharacters`
...
#### `search.rag.chunking.includeHeadings`
...
#### `search.rag.chunking.includeBreadcrumbs`
...
#### `search.rag.chunking.includeNav`
...
disable. `ignore` (or `VOCS_RAG_CACHE_IGNORE`) forces a full re-embed.

#### `search.rag.ui.debounceMs`
...
- **Type:** `string`
- **Default:** `` `{basePath}/api/search/rag` ``

Overrides the server endpoint path that embeds the query and searches the index.
...
Explicitly enable/disable RAG. Defaults to enabled when the `rag` object is present.

#### `search.retriever`

- **Type:** `Retriever.Adapter | object`

Adds AI search via a **managed** retriever instead of a Vocs-owned index. Delegates retrieval
to a backend (e.g. Cloudflare AI Search) queried at runtime (`/api/search/retrieve`) and fuses the
results with keyword search. Retriever adapters are kept **server-side only**.
...
Use `Retriever.cloudflare()` for a Cloudflare AI Search instance or `Retriever.from()` for a custom
backend. `Retriever.cloudflare()` accepts `instance` (required), `namespace`, `reranking`,
`retrievalType` (`'vector' | 'keyword' | 'hybrid'`), `rewriteQuery`, `scoreThreshold`, `filters`,
`mapResult`, and credential overrides (`accountId`, `apiToken`, `baseUrl`, `headers`).

:::tip
See [Search](/features/search) for the full guide — keyword-search recipes plus AI (RAG/retriever) search setup.
:::
