...
### Shared retriever options

Every `Retriever.*` constructor accepts these runtime/UI options alongside its provider-specific
options.

#### `Retriever` shared `topK`
...
#### `Retriever` shared `hybrid`

- **Type:** `boolean | { semanticWeight?: number; keywordWeight?: number }`
- **Default:** `false`

Fuse keyword (MiniSearch) and AI results into a single ranking. When `false`, AI results are appended
to keyword results instead. Pass an object to tune the fusion weights: `semanticWeight` (default
`0.7`) and `keywordWeight` (default `0.3`).

#### `Retriever` shared `endpoint`

- **Type:** `string`
- **Default:** `` `{basePath}/api/search` ``

Overrides the server endpoint path for the retriever.

#### `Retriever` shared `enabled`

- **Type:** `boolean`

Explicitly enable/disable AI search. Defaults to enabled when `ai.retriever` is present.

#### `Retriever` shared `ui.debounceMs`

- **Type:** `number`
- **Default:** `250`

Debounce before firing an AI search request from the search dialog.

### `Retriever.local`

Creates the self-owned retriever. Accepts the [shared options](#shared-retriever-options) plus the
options below (`embedding` is required, the rest are optional).
...
Accepts the [shared options](#shared-retriever-options) plus `instance` (required), `namespace`,
...
Cloudflare. Accepts the adapter as the first argument and the
[shared options](#shared-retriever-options) as the second.
