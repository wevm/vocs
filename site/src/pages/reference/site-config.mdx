...
Configures the Ask AI menu shown on pages.
...
Show the "Open in ChatGPT/Claude" actions. Set to `false` to hide outbound links to consumer AI tools. Copy page, view as Markdown, and Copy MCP URL stay available. The same flag also hides the ChatGPT and Claude buttons on [`Prompt`](/reference/components#prompt) blocks.
...
:::tip
See [Ask AI](/features/ask-ai) for menu actions and per-page frontmatter.
:::
