...
- **Type:** `false | readonly { href: string; type?: string }[]`
- **Default:** the bundled Geist font

Custom entries replace the default font hint; use `false` or `[]` to disable it.
Vocs renders font preloads with `crossorigin="anonymous"`. Font families and
`font-display` remain controlled by CSS. See [Fonts](/features/fonts#preloading-fonts).
