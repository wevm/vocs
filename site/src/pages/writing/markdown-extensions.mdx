...
`{info="…"}` adds an info icon after the filename. Hovering (or focusing) the icon shows a tooltip popover with the text. It works on both files and folders, and can be combined with `**bold**` highlighting or an inline comment.
