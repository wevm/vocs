    ...
    const keydown = (event: KeyboardEvent) => {
      ...
      if (isListening) {
        ...
        if (event.code === 'ArrowRight' && nextPage?.link) {
          ...
          if (isExternalLink) window.open(nextPage.link, 'noopener,noreferrer')
          else navigate(nextPage.link)
          index++
        }
        if (event.code === 'ArrowLeft' && prevPage?.link) {
          const isExternalLink = !prevPage.link.match(/^(\.*\/|#)/)
          if (isExternalLink) window.open(prevPage.link, 'noopener,noreferrer')
          else navigate(prevPage.link)
          index--
        }
      }
    }
