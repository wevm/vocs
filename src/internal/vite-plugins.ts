...
/**
 * Vite plugin that generates API reference pages from an OpenAPI spec.
 *
 * Loads the spec at startup, generates page files in the pages directory,
 * and provides operation data via a virtual module. Also merges sidebar
 * items into the config.
 *
 * @param config - Vocs configuration.
 * @returns Plugin.
 */
export function openapi(config: Config.Config): PluginOption {
  ...
  if (!openapiConfig) return { name: 'vocs:openapi' }

  const { spec, basePath = '/api' } = openapiConfig
  const pagesDir = path.resolve(config.rootDir, config.srcDir, config.pagesDir)
  const generatedDir = path.resolve(pagesDir, basePath.replace(/^\//, ''))

  const virtualModuleId = 'virtual:vocs/openapi-data'
  const resolvedVirtualModuleId = `\0${virtualModuleId}`
  ...
  async function loadAndGenerate() {
    ...
    logger.info('Loading OpenAPI spec...', { timestamp: true })
    const doc = await OpenApi.loadSpec({ spec })
    operations = OpenApi.extractOperations(doc)
    logger.info(`Found ${operations.length} operations in OpenAPI spec`, { timestamp: true })

    // Merge sidebar items into config
    const sidebarItems = OpenApi.buildSidebarItems(operations, basePath)
    const currentSidebar = config.sidebar
    if (currentSidebar && typeof currentSidebar === 'object' && !Array.isArray(currentSidebar)) {
      ;(currentSidebar as Record<string, unknown>)[`${basePath}/`] = {
        backLink: true,
        items: sidebarItems,
      }
    } else {
      ;(config as Record<string, unknown>).sidebar = {
        ...(currentSidebar && typeof currentSidebar === 'object' ? currentSidebar : {}),
        [`${basePath}/`]: {
          backLink: true,
          items: sidebarItems,
        },
    ...
    await fs.writeFile(
      ...
      generateIndexPage(operations, basePath, doc.info?.title),
      { encoding: 'utf-8' },
    )

    // Write operation pages grouped by tag
    const tagGroups = new Map<string, OpenApi.Operation[]>()
    ...
    for (const [tag, ops] of tagGroups) {
      const tagSlug = OpenApi.getTagSlug(tag)
      const tagDir = path.join(generatedDir, tagSlug)
    ...
    // Write .gitignore in generated dir
    await fs.writeFile(
      path.join(generatedDir, '.gitignore'),
      '# Auto-generated by vocs openapi plugin\n*\n',
      { encoding: 'utf-8' },
    )

    specLoaded = true
    logger.info(`Generated ${operations.length} OpenAPI pages at ${basePath}/`, {
      timestamp: true,
    })
  }
  ...
  return {
    ...
    load(id) {
      if (id === resolvedVirtualModuleId) {
        return `export const operations = ${JSON.stringify(operations)}`
      }
      return
    },
    async configureServer(server) {
      // Watch for spec file changes (if local)
      if (!spec.startsWith('http://') && !spec.startsWith('https://')) {
        server.watcher.on('change', async (changedPath) => {
          const specPath = path.resolve(config.rootDir, spec)
          if (changedPath !== specPath) return
          specLoaded = false
          await loadAndGenerate()
          server.restart()
        })
...
function generateOperationPage(op: OpenApi.Operation): string {
  const json = JSON.stringify(op)
  return `// Auto-generated by vocs openapi plugin — do not edit
import { Layout } from 'vocs'
import { OpenApiPage } from 'vocs/react/internal/OpenApiPage'
...
export default function Page() {
  return (
    <Layout>
      <OpenApiPage operation={operation} />
    </Layout>
  )
}
...
function generateIndexPage(
  ...
  const defaultColor = 'vocs:bg-gray-500/10 vocs:text-gray-600'

  const sections = Array.from(tagGroups.entries())
    .map(([tag, ops]) => {
      const tagSlug = OpenApi.getTagSlug(tag)
      const links = ops
        .map((op) => {
          const slug = OpenApi.getOperationSlug(op)
          const label = op.summary ?? op.operationId
          const color = methodColorMap[op.method] ?? defaultColor
          return `          <li key="${op.operationId}"><a href="${basePath}/${tagSlug}/${slug}" data-v className="vocs:flex vocs:items-center vocs:gap-2 vocs:no-underline"><span className="vocs:text-[10px] vocs:font-mono vocs:font-semibold vocs:uppercase vocs:w-14 vocs:text-center vocs:px-1.5 vocs:py-0.5 vocs:rounded-md ${color}">${op.method.toUpperCase()}</span><span>${label}</span></a></li>`
        })
        .join('\n')
      return `        <section className="vocs:mb-8">
          <h2 className="vocs:text-heading vocs:text-xl vocs:font-medium vocs:mb-3" data-v>${tag}</h2>
          <ul className="vocs:space-y-1.5 vocs:list-none vocs:pl-0">
${links}
          </ul>
        </section>`
    })
  ...
  return (
    <Layout>
      <article data-v-content>
        <hgroup data-v>
          <h1 data-v>${title ?? 'API Reference'}</h1>
          <p>${operations.length} endpoints</p>
        </hgroup>
${sections}
      </article>
    </Layout>
