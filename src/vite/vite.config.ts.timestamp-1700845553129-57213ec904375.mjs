...
async function resolveVocsConfig2(parameters = {}) {
...
function virtualConfig() {
  ...
  return {
    ...
    async configureServer(server) {
      const { configPath } = await resolveVocsConfig2();
      if (configPath) {
        server.watcher.add(configPath);
        server.watcher.on("change", async () => {
          server.ws.send("vocs:config", (await resolveVocsConfig2()).config);
        });
      }
    },
    resolveId(id) {
    ...
    async load(id) {
      if (id === resolvedVirtualModuleId) {
        const { config } = await resolveVocsConfig2();
    ...
    async configureServer(server) {
      ...
      if (sourceFiles.length) {
        for (const sourceFile of sourceFiles) {
          const path2 = sourceFile.getFilePath();
          server.watcher.add(path2);
          server.watcher.on("change", async () => {
